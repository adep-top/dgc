...
export const API = {
  LOGIN: '/v1/login',
  ME: '/v1/me',
  CREATE_ROOM: '/v1/rooms',          // POST 创建；GET 大厅房间列表
  ROOM: (id) => `/v1/rooms/${id}`,
  JOIN: (id) => `/v1/rooms/${id}/join`,
  LEAVE: (id) => `/v1/rooms/${id}/leave`, // POST 主动离开房间（解除「一人一房」归属）
  QUICK_MATCH: '/v1/match',          // POST 陌生人快速匹配
  RANKING: '/v1/ranking',            // GET 积分排行榜
  MATCHES: '/v1/matches',            // GET 我的战绩列表
  MATCH_DETAIL: (id) => `/v1/matches/${id}`, // GET 对局详情（含落子序列）
  // ---- 异步对战（每步落库 + 订阅消息提醒，不依赖长连接）----
  ASYNC_CREATE: '/v1/async/games',                    // POST 创建异步对局
  ASYNC_LIST: '/v1/async/games',                      // GET 我的异步对局列表
  ASYNC_GAME: (id) => `/v1/async/games/${id}`,        // GET 获取对局状态
  ASYNC_JOIN: (id) => `/v1/async/games/${id}/join`,   // POST 加入等待中的对局
  ASYNC_MOVE: (id) => `/v1/async/games/${id}/move`,   // POST 落子
  ASYNC_SUBSCRIBE: (id) => `/v1/async/games/${id}/subscribe`, // POST 登记订阅模板
  // ---- 小程序码邀请（带房间号的小程序码图片，扫码直接进房）----
  WXACODE: (roomId) => '/v1/wxacode/' + roomId,   // GET 获取小程序码图片（二进制 image/jpeg）
};
